...
describe "options: " do
  context "overruling the basepath" do
    ...
    it "retrieves the given base-path on /swagger_doc" do
      get '/swagger_doc'
      last_response.body.should == "{:apiVersion=>\"0.1\", :swaggerVersion=>\"1.1\", :basePath=>\"http://www.breakcoregivesmewood.com\", :operations=>[], :apis=>[{:path=>\"/swagger_doc/.{format}\"}]}"
    end

    it "retrieves the same given base-path for mounted-api" do
      get '/swagger_doc/something'
      last_response.body.should == "{:apiVersion=>\"0.1\", :swaggerVersion=>\"1.1\", :basePath=>\"http://www.breakcoregivesmewood.com\", :resourcePath=>\"\", :apis=>[{:path=>\"/something.{format}\", :operations=>[{:notes=>nil, :summary=>\"this gets something\", :nickname=>\"GET-something---format-\", :httpMethod=>\"GET\", :parameters=>[]}]}]}"
    end
  end
  ...
  context "overruling the version" do
    ...
    it "retrieves the api version on /swagger_doc" do
      get '/swagger_doc'
      last_response.body.should == "{:apiVersion=>\"101\", :swaggerVersion=>\"1.1\", :basePath=>\"http://example.org\", :operations=>[], :apis=>[{:path=>\"/swagger_doc/.{format}\"}]}"
    end
  ...
  context "overruling the mount-path" do
    ...
    it "retrieves the given base-path on /api_doc" do
      get '/api_doc'
      last_response.body.should == "{:apiVersion=>\"0.1\", :swaggerVersion=>\"1.1\", :basePath=>\"http://example.org\", :operations=>[], :apis=>[{:path=>\"/api_doc/.{format}\"}]}"
    end
